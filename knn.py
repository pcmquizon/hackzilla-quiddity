...
if intent == "find-meal":
  ...
  for distance, user in nearest:
    cursor.execute('''SELECT order_foods.food_id, count(order_foods.food_id), food.name, food.price from user_order
    ...
    for x in cursor.fetchall():
      food, frequency, name, price = x
      if food in result:
        result[food][0] += frequency
      else:
        result[food] = [frequency, name, price]
  ...
  for x in result:
    ...
    json_list.append({'food_id':food_id, 'freq':freq, 'name': name, 'price': price})
  ...
  result = {}
  for distance, user in nearest:
    cursor.execute('''SELECT order_foods.food_id, count(order_foods.food_id) as freq, food.name, food.price
              FROM user_order
              JOIN order_foods ON user_order.id = order_foods.order_id
              JOIN food on food.id = order_foods.food_id
              and user_order.resto_id = food.resto_id
